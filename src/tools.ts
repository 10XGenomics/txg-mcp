    ...
    switch (name) {
      ...
      case 'create_cellranger_multi_analysis': {
        const schema = z.object({
          ...
          product_version: z.string().optional().describe("Specific Cell Ranger version")
        });
        ...
        const command = ['analyses', 'create', 'cellranger', 'multi',
                        ...
                        '--csv', params.csv_path,
                        '--assumeyes', '--wait-completion=false'];

        if (params.project_id) command.push('--project-id', params.project_id);
        else if (params.project_name) command.push('--project-name', params.project_name);
      ...
      case 'create_cellranger_count_analysis': {
        const schema = z.object({
          ...
          fastqs: z.array(z.string()).describe("List of FASTQ file paths or IDs"),
      ...
      case 'update_project': {
        ...
        const params = schema.parse(args);
        const command = ['projects', 'update', params.project_id, '--assumeyes'];
      ...
      case 'update_reference': {
        ...
        const params = schema.parse(args);
        const command = ['references', 'update', params.reference_id, '--assumeyes'];
        if (params.name) command.push('--name', params.name);
        if (params.description) command.push('--description', params.description);
