...
import { ValidationError, CommandNotAllowedError, ExecutionError } from '../types/index.js';
...
function validateCommand(commandParts) {
...
async function executeTxgCommand(commandParts) {
    return new Promise((resolve, reject) => {
        try {
            // Validate command
            validateCommand(commandParts);
            // Sanitize all arguments
            const sanitizedParts = commandParts.map((part) => {
            ...
            child.on('error', (error) => {
                if ('code' in error && error.code === 'ENOENT') {
                    reject(new ExecutionError(`TXG executable not found at ${CONFIG.txgExecutable}. Please ensure TXG CLI is installed.`));
                }
                else {
...
const TOOLS = [
    {
        ...
        handler: async (args) => {
            ...
            const command = ['analyses', 'create', 'cellranger', 'multi',
                '--analysis-name', typedArgs.analysis_name,
                '--csv', typedArgs.csv_path,
                '--assumeyes'
            ];
            if (typedArgs.project_id)
                command.push('--project-id', typedArgs.project_id);
            else if (typedArgs.project_name)
                command.push('--project-name', typedArgs.project_name);
            if (typedArgs.description)
                command.push('--description', typedArgs.description);
            if (typedArgs.product_version)
                command.push('--product-version', typedArgs.product_version);
    ...
    },
    {
        ...
        handler: async (args) => {
            const typedArgs = args;
            const command = ['analyses', 'create', 'cellranger', 'count',
                '--analysis-name', typedArgs.analysis_name,
            ...
            if (typedArgs.project_id)
                ...
                command.push('--project-name', typedArgs.project_name);
            if (typedArgs.expect_cells)
                command.push('--expect-cells', String(typedArgs.expect_cells));
            if (typedArgs.chemistry)
                command.push('--chemistry', typedArgs.chemistry);
            return await executeTxgCommand(command);
        }
    },
    {
        ...
        handler: async (args) => {
            const typedArgs = args;
            const command = ['analyses', 'create', 'cellranger', 'aggr',
                '--analysis-name', typedArgs.analysis_name,
                '--csv', typedArgs.csv_path,
                '--assumeyes'
            ];
            if (typedArgs.project_id)
                ...
                command.push('--project-name', typedArgs.project_name);
            if (typedArgs.normalize)
                command.push('--normalize', typedArgs.normalize);
            if (typedArgs.description)
                command.push('--description', typedArgs.description);
            return await executeTxgCommand(command);
        }
    ...
    },
    {
        ...
        handler: async (args) => {
            const typedArgs = args;
            const command = ['projects', 'create', '--name', typedArgs.name, '--assumeyes'];
            if (typedArgs.description)
                command.push('--description', typedArgs.description);
            return await executeTxgCommand(command);
        }
    },
    {
        ...
        handler: async (args) => {
            const typedArgs = args;
            const command = ['projects', 'update', typedArgs.project_id, '--assumeyes'];
            if (typedArgs.name)
                command.push('--name', typedArgs.name);
            if (typedArgs.description)
                command.push('--description', typedArgs.description);
            return await executeTxgCommand(command);
        }
    ...
    },
    {
        ...
        handler: async (args) => {
            const typedArgs = args;
            const command = ['references', 'update', typedArgs.reference_id, '--assumeyes'];
            if (typedArgs.name)
                command.push('--name', typedArgs.name);
            if (typedArgs.description)
                command.push('--description', typedArgs.description);
